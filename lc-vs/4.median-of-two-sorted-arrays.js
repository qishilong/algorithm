...
var findMedianSortedArrays = function (nums1, nums2) {
  ...
  while (length1 > 0 || length2 > 0) {
    const val1 = nums1[length1 - 1] !== undefined ? nums1[length1 - 1] : -Infinity;
    const val2 = nums2[length2 - 1] !== undefined ? nums2[length2 - 1] : -Infinity;

    const resLastIndex = length1 + length2 - 1;

    if (val1 > val2) {
      res[resLastIndex] = val1;
      --length1;
    } else if (val1 === val2) {
      res[resLastIndex] = val1;
      --length1;
      res[resLastIndex - 1] = val2;
      --length2;
    } else {
      res[resLastIndex] = val2;
      --length2;
    }
  }
